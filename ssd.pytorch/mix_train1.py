...
def train():
    ...
    if args.resume:
        ...
        print('Loading base network...    ', args.save_folder + args.basenet)
    ...
    print("------------------------------------------\n"
          "          Mixup only label                \n"
          "------------------------------------------\n")
    log_folder = './results/' + net.__class__.__name__ + '/' + optimizer.__class__.__name__ + '/mixup/' + "mix_label2" + '/'
    ...
    for iteration in range(args.start_iter, cfg['max_iter']):
        ...
        lam = lam if lam < 0.5 else (1.0 - lam)
        ##  mixup
        ...
        if iteration % 10 == 0:
            ...
            print('Epoch: {} |iter '.format(epoch) + repr(iteration) + ' || Loss_l:%.4f  loss_c:%.4f ' % (loss_l.data[0], loss_c.data[0]) + "|| lr: {}".format(lr), end=' ')

        if iteration != 0 and iteration % 5000 == 0:
            print('Saving state, iter:', iteration)
            torch.save(ssd_net.state_dict(), 'weights/mix_label/ssd300_voc_' +
                       repr(iteration) + '.pth')
