...
from layers.modules.multibox_loss_iou import MultiBoxLoss as MultiBoxLoss_u
...
# ---------------------------------------------------------------------------------
parser.add_argument('--save_folder', default='./results/test/IOU',
...
criterion = MultiBoxLoss_u(num_classes, 0.5, True, 0, True, 3, 0.5, False)
    ...
    for iteration in range(start_iter, max_iter):
        if iteration % epoch_size == 0:
            ...
            if (epoch % 10 == 0 and epoch > 0) or (epoch % 5 == 0 and epoch > 200):
                print("Save State , Epoch:{} ".format(epoch))
        ...
        if iteration % 10 == 0:
            print('Epoch:' + repr(epoch) + ' || epochiter: ' + repr(iteration % epoch_size) + '/' + repr(epoch_size)
                  + '|| Totel iter ' +
                  repr(iteration) + ' || L: %.4f C: %.4f Iou: %.4f ||' % (
                      loss_l.item(), loss_c.item(), loss_iou.item()) +
